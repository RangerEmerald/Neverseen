...
let db = new sqlite3.Database('src/databases/database.db');
...
async function addScore(id, points = 1) {
    ...
    if (userscore == undefined) {
        db.run(`INSERT INTO scores(id, points) VALUES(?, ?)`, [id, points], function (err) {
            if (err) throw err;
            scoreMapCache.set(id, 1);
        });
    } else {
        db.run(`UPDATE scores SET points = ? WHERE id = ?`, [userscore + points, id], function (err) {
            if (err) throw err;
            scoreMapCache.set(id, userscore + 1);
        });
    }
}
...
async function deleteScore(id, points) {
    ...
    if (userscore == undefined) return false;
        ...
        db.run(`UPDATE scores SET points = ? WHERE id = ?`, [userscore - points, id], function (err) {
            if (err) throw err;
            scoreMapCache.set(id, userscore - points);
        })
    } else {
